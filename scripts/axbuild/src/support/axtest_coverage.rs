...
/// Replace the QEMU success regex so that test-specific success markers cannot
/// terminate QEMU before coverage extraction. The capture requests a monitor
/// quit after extraction and then publishes `AXTEST_COVERAGE_DONE`.
///
/// The returned regexes remain the host-side test success contract. Coverage
/// completion is generated by the outer capture layer, so the nested output
/// capture used to verify the test result cannot observe it.
                    ...
                    match pipe.read(&mut buf) {
                        ...
                        Ok(n) => {
                            ...
                            if let Ok(mut state) = reader_state.lock() {
                                ...
                                // If coverage was just extracted, publish the
                                // completion evidence after requesting monitor quit.
...
mod tests {
    ...
    #[test]
    fn coverage_wait_marker_does_not_replace_test_success_contract() {
