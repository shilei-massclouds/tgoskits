"""Adapter specification for controlled eventfd blocking scenarios."""
...
from host_runtime import HostRecordResult, find_or_build_host_oracle
...
def record_host_stable(
    ...
    with tempfile.TemporaryDirectory(
        prefix=".eventfd-blocking-host-", dir=trace_path.parent
...
def _seed_inputs(workspace: Path):
    checked = (
        workspace
        / "test-suit/starryos/qemu/eventfd-linux-oracle/c/corpus/eventfd-blocking.ops"
    )
...
def _mutate(rng: object, parent: bytes, donor: bytes):
    ...
    if (
        ...
        is not blocking_mutation.CandidateClassification.EXECUTABLE
    ):
        return None
...
SPEC = AdapterSpec(
    adapter_id="eventfd-blocking-v1",
    adapter_version=1,
    corpus_version=2,
    ...
    campaign=CampaignLayout(Path("coverage/eventfd-blocking-oracle-fuzz")),
    qemu=QemuSpec(
