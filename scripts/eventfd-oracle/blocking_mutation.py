"""Lifecycle-aware mutation for the controlled eventfd model."""
...
def mutate_document(
    rng: blocking_generator.CampaignRng,
...
def _apply_mutation(
    ...
    generated = blocking_generator.generate_scenario(
        rng, story=(MUTATION_KINDS.index(kind) + attempt) % blocking_generator.STORY_COUNT
    )
    if kind in ("lifecycle-replace", "parameter"):
        ...
        scenarios[index] = rng.choice(donor.scenarios)
    else:
        scenarios[index] = generated
    return ScenarioDocument(scenarios, version=2)
