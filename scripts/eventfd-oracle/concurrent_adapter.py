...
RAW_MAGIC = b"EVFDRUN4"
TRACE_MAGIC = b"EVFDORC4"
...
def _seed_inputs(workspace: Path):
    checked = workspace / "test-suit/starryos/qemu/eventfd-linux-oracle/c/corpus/eventfd-concurrent.ops"
...
SPEC = AdapterSpec(
    adapter_id="eventfd-concurrent-v1",
    adapter_version=1,
    corpus_version=4,
    generator_version=concurrent_generator.GENERATOR_VERSION,
    artifacts=ArtifactLayout("eventfd.ops", "linux.trace", "eventfd-linux-oracle"),
    campaign=CampaignLayout(Path("coverage/eventfd-concurrent-v1-oracle-fuzz")),
    qemu=QemuSpec(
