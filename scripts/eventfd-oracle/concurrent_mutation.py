"""Lifecycle-preserving mutation for concurrent eventfd stories."""
...
def mutate_document(rng, parent, donor, *, requested_kind=None):
    ...
    document = ScenarioDocument(scenarios, version=4)
    validate_entry_limits(document)
    encoded = serialize_document(document).encode("utf-8")
    digest = canonical_digest(document)
    if digest == canonical_digest(parent):
        document = ScenarioDocument((concurrent_generator.generate_scenario(rng),), version=4)
        validate_entry_limits(document)
        encoded = serialize_document(document).encode("utf-8")
        digest = canonical_digest(document)
    classification = (
