"""Controlled eventfd poll scenario IR and canonical v3 codec."""
...
BlockingOperation = Union[StartPoll, AssertPending, Join]
Operation = Union[simple.Operation, BlockingOperation]
WorkerCall = controlled_actor.WorkerCall


class ResourceState:
    """Prove one poll worker blocks first and can complete before join."""
    ...
    def _start_worker(self, operation: StartPoll) -> None:
        def identify_event() -> int:
            ...
            if operation.events == POLLIN and event.count != 0:
                raise ScenarioCodecError(
                    "blocking-proof", "worker poll would be initially ready"
                )
            if operation.events == POLLOUT and event.count != MAX_COUNTER:
                raise ScenarioCodecError(
                    "blocking-proof", "worker poll would be initially ready"
                )
    ...
    def _join_worker(self, operation: Join) -> None:
        del operation
        self._apply_worker_transition(
            lambda: self._worker_lifecycle.join(lambda _worker: None)
        )
    ...
    def _apply_worker_transition(transition: Callable[[], object]):
        try:
            ...
            category = (
                "actor-lifecycle"
                if error.kind
                is controlled_actor.WorkerLifecycleErrorKind.LIFECYCLE
                else "blocking-proof"
            )
...
def parse_document(encoded: Union[bytes, str]) -> ScenarioDocument:
    """Parse and validate one canonical-format-compatible v3 document."""
...
def validate_document(document: ScenarioDocument) -> None:
    ...
    for scenario_index, source_scenario in enumerate(document.scenarios):
        if (
            not source_scenario.operations
            or len(source_scenario.operations) > MAX_OPS_PER_SCENARIO
        ):
            raise ScenarioCodecError(
...
def _parse_operation(fields: Tuple[str, ...], line_number: int) -> Operation:
    name = fields[0]
    values = fields[1:]
    if name == "start-poll" and len(values) == 3:
        return StartPoll(
            _worker_actor(values[0], line_number),
            simple._slot(values[1], line_number),
            simple._integer_from_values(values[2], POLL_EVENTS, line_number),
        )
...
def _validate_operation_fields(operation: Operation) -> None:
    ...
    if parsed != operation:
        raise ScenarioCodecError("operation", f"invalid {type(operation).__name__}")
