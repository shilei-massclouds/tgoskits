...
class ImportStore:
    ...
    def create_job(
        ...
        *,
        ...
        try:
            ...
            for directory in (sources_dir, conversions_dir, inputs_dir, temporary):
    ...
    def load_resumable_jobs(self) -> List[ImportJob]:
        ...
        for path in sorted(self.jobs_dir.iterdir(), key=lambda item: item.name):
            ...
            if job.metadata["state"] not in {"completed", "failed"} or (
    ...
    def record_batch_result(
        ...
        *,
        ...
        batch["state"] = "failed" if result_category.endswith("failure") else "completed"
