"""Adapter specification for controlled pipe blocking poll scenarios."""
...
def record_host_stable(
    ...
    return record_stable_host(
        ...
        scenario_path,
        trace_path,
        temporary_prefix=".pipe-blocking-v2-host-",
    )
...
def _seed_inputs(workspace: Path):
    yield (
        workspace
        / "test-suit/starryos/qemu/pipe-linux-oracle/c/corpus/pipe-blocking-poll.ops"
    ).read_bytes()
    for seed in range(5):
...
def _reduction_candidates(document: object):
    return (
        candidate.document for candidate in poll_reducer.reduction_candidates(document)
    )
