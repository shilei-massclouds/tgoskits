"""Deterministic reduction that preserves controlled pipe poll lifecycles."""
...
def reduction_candidates(
    ...
    *,
    ...
    """Yield unique strictly smaller documents accepted by the v6 validator."""
...
def _simpler_operations(operation):
    ...
    if isinstance(operation, (Read, Write)):
        for length in (0, 1, 16, 4095, 4096):
    ...
    if isinstance(operation, Pipe2) and operation.flags == O_NONBLOCK:
        replacements.append((replace(operation, flags=0), "flags=0"))
    if isinstance(operation, (StartPoll, AssertPending, Join)):
        pass
    return replacements
