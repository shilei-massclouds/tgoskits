...
class PersistentCorpusRegressionTests(unittest.TestCase):
    ...
    def test_coverage_state_is_restored_only_for_the_same_elf(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(
                ...
                {
                    f"{first_digest}-pipe-poll-v4",
                    f"{second_digest}-pipe-poll-v4",
                },
            )
    ...
    def test_coverage_state_is_isolated_by_target_set_and_loads_legacy_v1(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            store.save_coverage_regions(
                elf,
                {"os/StarryOS/kernel/src/syscall/fs/io.rs:4:1"},
                pipe_coverage.VECTOR_TARGET_SET_ID,
            )
            ...
            self.assertEqual(
                store.load_coverage_regions(elf, pipe_coverage.VECTOR_TARGET_SET_ID),
                {"os/StarryOS/kernel/src/syscall/fs/io.rs:4:1"},
            )
    ...
    def test_coverage_state_symlink_fails_closed(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            external.write_text("{}")
            state = store.coverage_state_dir / f"{digest}-pipe-poll-v4.json"
    ...
    def test_current_corpus_loader_accepts_generator_v2_through_v5_but_rejects_v1(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            workspace = Path(temporary_directory)
            store = corpus.CorpusStore(workspace)
            ...
            for compatible_version in ("2", "3", "4", "5"):
